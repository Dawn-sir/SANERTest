...
class codebert_tabular(SemiDataset,TabularMixin):
    def __init__(
        self,
        ...
        self.dataset=pd.read_csv('E:\桌面\LAMDA-SSL-master\method_whole_codebert.csv')
    ...
    def _init_dataset(self):
        selected_cols = ['codebert', 'cvss2_severity']
        df_tmp = self.dataset[selected_cols].copy()
        column_name = 'cvss2_severity'
        mapping = {'HIGH': 0, 'MEDIUM': 1, 'LOW': 2}
        df_tmp[column_name] = df_tmp[column_name].map(mapping)
        codebert_arr = df_tmp['codebert'].values
        ...
        X = X_array
        y = df_tmp['cvss2_severity'].values
