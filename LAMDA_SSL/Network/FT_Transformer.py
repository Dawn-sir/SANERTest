...
class FT_Transformer(nn.Module):
    def __init__(
            ...
            layer = nn.ModuleDict(
                {
                    ...
                    'norm1': make_normalization(self),
                }
            )
            if not prenormalization or layer_idx:
                layer['norm0'] = make_normalization(self)
            if kv_compression and self.shared_kv_compression is None:
                layer['key_compression'] = make_kv_compression(self)
                if kv_compression_sharing == 'headwise':
                    layer['value_compression'] = make_kv_compression(self)
                else:
        ...
        def get_nonglu_activation_fn(self, name: str) -> ty.Callable[[Tensor], Tensor]:
            return (
                F.relu
                if name == 'reglu'
                else F.gelu
                if name == 'geglu'
                else get_activation_fn(self, name)
            )
        def get_activation_fn(self, name: str) -> ty.Callable[[Tensor], Tensor]:
            return (
                reglu
                if name == 'reglu'
                else geglu
        ...
        self.activation = get_activation_fn(self, activation)
        self.last_activation = get_nonglu_activation_fn(self, activation)
        self.prenormalization = prenormalization
        self.last_normalization = make_normalization(self) if prenormalization else None
